...
@section('content')
    <div class="bg-white rounded-[3rem] shadow-2xl shadow-[#001BB7]/5 overflow-hidden border border-[#0046FF]/5">
        ...
        @auth
            @if(auth()->user()->isAdmin()) <div class="mb-6 p-4 bg-gray-50 border rounded-lg flex justify-between items-center">
                    <div>
                        <h3 class="font-bold text-gray-700">AI Review Analysis</h3>
                        <p class="text-sm text-gray-500">Generate a fresh summary of all customer reviews.</p>
                    </div>
        ...
        <div id="ai-summary-container">
            @if($book->ai_summary)
                {{-- Existing Summary UI --}}
                <div class="my-8 p-6 bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100 rounded-xl shadow-sm">
                    <div class="flex items-center gap-2 mb-3">
                        <svg class="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                        </svg>
                        <h3 class="text-lg font-bold text-purple-900">AI Review Summary</h3>
                    </div>
                    <div class="prose prose-purple text-gray-700 leading-relaxed">
                        {!! nl2br(e($book->ai_summary)) !!}
                    </div>
                    <div class="mt-4 text-xs text-gray-400">Generated by PageTurner AI</div>
                </div>
            @elseif(Cache::has("ai_processing_{$book->id}"))
                {{-- NEW: The "Smart Placeholder" --}}
                <div class="my-8 p-6 bg-gray-50 border-2 border-dashed border-purple-200 rounded-xl animate-pulse">
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-5 h-5 bg-purple-200 rounded-full"></div>
                        <h3 class="text-lg font-bold text-purple-300 uppercase tracking-tighter">AI is analyzing feedback...</h3>
                    </div>
                    <div class="space-y-2">
                ...
                {{-- Polling Script: Automatically refreshes the section when data is ready --}}
                <script>
                        ...
                        fetch('{{ route('books.ai-status', $book->id) }}')
                            ...
                            .then(data => {
                                if (data.ready) {
                                    ...
                                    window.location.reload(); // Refresh once to clear the cache and show the result
                                }
                            });
