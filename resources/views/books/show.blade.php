    ...
    <div class="bg-white rounded-[3rem] shadow-2xl shadow-[#001BB7]/5 overflow-hidden border border-[#0046FF]/5">
        ...
        @auth
            @if(auth()->user()->isAdmin()) 
                <div class="m-8 lg:m-12 p-8 bg-[#F5F1DC]/50 border border-[#001BB7]/10 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-6 shadow-inner">
                    <div>
                        <h3 class="font-black text-[#001BB7] uppercase tracking-tighter text-2xl">Admin: AI Intelligence</h3>
                        <p class="text-[#0046FF]/60 font-bold mt-1">Generate a fresh summary of all customer perspectives.</p>
                    </div>
                    
                    <form action="{{ route('books.analyze-reviews', $book->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="whitespace-nowrap px-8 py-4 bg-[#001BB7] text-[#F5F1DC] rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] hover:bg-[#0046FF] transition-all shadow-xl shadow-[#001BB7]/20 active:scale-95">
                            Run AI Analysis
                        </button>
                    </form>
                </div>
        ...
        {{-- AI Summary Output --}}
        <div id="ai-summary-container" class="px-8 lg:px-12">
            @if($book->ai_summary)
                <div class="my-4 p-10 bg-gradient-to-br from-[#001BB7] to-[#0046FF] rounded-[2.5rem] shadow-2xl shadow-[#001BB7]/20 relative overflow-hidden">
                    <div class="absolute -right-10 -top-10 text-white/5">
                        <svg class="w-64 h-64" fill="currentColor" viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                    </div>
                    
                    <div class="relative z-10">
                        <div class="flex items-center gap-4 mb-6">
                            <div class="p-3 bg-white/10 rounded-2xl backdrop-blur-sm shadow-inner">
                                <svg class="w-6 h-6 text-[#F5F1DC]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                                </svg>
                            </div>
                            <h3 class="text-2xl font-black text-[#F5F1DC] uppercase tracking-tighter">AI Review Summary</h3>
                        </div>
                        <div class="text-white/90 text-lg font-medium leading-relaxed italic">
                            {!! nl2br(e($book->ai_summary)) !!}
                        </div>
                        <div class="mt-8 text-[10px] font-black text-white/40 uppercase tracking-[0.3em]">Generated by PageTurner AI</div>
                    </div>
                </div>
            @elseif(Cache::has("ai_processing_{$book->id}"))
                <div class="my-4 p-10 bg-[#F5F1DC]/30 border-2 border-dashed border-[#001BB7]/20 rounded-[2.5rem] animate-pulse">
                    <div class="flex items-center gap-4 mb-6">
                        <div class="w-12 h-12 bg-[#001BB7]/20 rounded-2xl"></div>
                        <h3 class="text-xl font-black text-[#001BB7]/40 uppercase tracking-tighter">AI is analyzing feedback...</h3>
                    </div>
                    <div class="space-y-4">
                        <div class="h-4 bg-[#001BB7]/10 rounded-full w-3/4"></div>
                        <div class="h-4 bg-[#001BB7]/10 rounded-full w-5/6"></div>
                        <div class="h-4 bg-[#001BB7]/10 rounded-full w-1/2"></div>
                    </div>
                </div>
                
                <script>
                    let pollInterval = setInterval(() => {
                        fetch('{{ route('books.ai-status', $book->id) }}')
        ...
        <div class="md:flex mt-8 border-t border-[#0046FF]/5">
