...
@section('content')
        ...
        <div class="md:flex">
            ...
            <div class="md:w-2/3 p-10 lg:p-16">
                ...
                @auth
                    ...
                    <div x-data="{ quantity: 1 }" class="mt-10 p-6 bg-indigo-50 rounded-[2rem] border border-indigo-100">
                        <form action="{{ route('orders.store') }}" method="POST">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <button type="submit" class="flex-1 bg-amber-400 text-amber-950 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-amber-500 transition shadow-lg shadow-amber-100 active:scale-95">
                                    Add to Cart
                                </button>
                            </div>
                        </form>
                ...
                @else
                    <div class="mt-10">
                        <a href="{{ route('login') }}" class="block text-center bg-gray-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-black transition">Login to Add to Cart</a>
                    </div>
                @endauth
    ...
    {{-- Reviews Section --}}
    <div class="mt-20">
        <h2 class="text-3xl font-black text-gray-900 tracking-tight mb-10">Community Reviews</h2>
                ...
                <div x-data="{ isEditing: {{ $existingReview ? 'false' : 'true' }} }" class="mb-12">
                    @if($existingReview)
                        <div x-show="!isEditing" class="bg-indigo-50 rounded-3xl p-8 flex flex-col md:flex-row justify-between items-center gap-6 border border-indigo-100">
                            ...
                            <button @click="isEditing = true" class="bg-white text-indigo-600 px-6 py-3 rounded-2xl font-black uppercase tracking-widest text-xs shadow-sm hover:shadow-md transition">Edit Review</button>
                        </div>
                    @endif

                    <div x-show="isEditing" class="bg-white rounded-3xl shadow-sm border border-indigo-100 p-8">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="font-black text-indigo-600 uppercase text-xs tracking-widest">{{ $existingReview ? 'Update Your Review' : 'Write a Review' }}</h3>
                            @if($existingReview)
            ...
            @else
                @if(!Auth::user()->isAdmin())
                    <div class="bg-gray-50 border border-dashed border-gray-300 rounded-3xl p-8 mb-12 text-center">
                        <p class="text-gray-500 font-bold text-sm">Only customers who have <span class="text-indigo-600 font-black">completed their order</span> can leave reviews.</p>
                    </div>
                @endif
            @endif
        @endauth
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @forelse($book->reviews as $review)
                <div class="bg-white rounded-3xl border border-gray-100 p-8 hover:shadow-lg transition-all group">
                    <div class="flex justify-between items-start">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="font-black text-gray-900 leading-none">{{ $review->user->getFullName() }}</p>
            ...
            @empty
                <div class="md:col-span-2 py-16 text-center text-gray-400 font-black uppercase tracking-widest border-2 border-dashed border-gray-100 rounded-[3rem]">No reviews yet.</div>
            @endforelse
        </div>
    </div>
@endsection